...
function buildPayload(intent) {
    ...
    if (entities.appName)
        payload.appName = entities.appName;
    if (entities.contactName)
        payload.contactName = entities.contactName;
    if (entities.reminderText)
        payload.reminderText = entities.reminderText;
...
function resolveAction(intent) {
    var action = ACTION_NAME_BY_INTENT[intent.type];
    if (!action) {
        return {
            action: 'none',
            executable: false,
